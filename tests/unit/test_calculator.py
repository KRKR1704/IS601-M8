...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1000000000, 1, 1000000001),  # Test adding a very large integer and 1
        (1, 2.0, 3.0),                # Test mixing int and float
    ],
    ids=[
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (3, 5, -2),           # Test subtracting larger from smaller (negative result)
        (1.0, 0.5, 0.5),      # Test subtracting floats with simple fractional result
    ],
    ids=[
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (1000000, 1000, 1000000000),  # Test multiplying large integers
        (0.5, 0.2, 0.1),              # Test multiplying small floats
    ],
    ids=[
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (7, 2, 3.5),           # Test division producing a non-integer float
        (-7, 2, -3.5),         # Test negative dividend producing non-integer float
    ],
    ids=[
...
def test_power(a: Number, b: Number, expected: float) -> None:
    """
    Test the 'power' function for various exponents and bases.
    """
    result = power(a, b)
    assert result == expected, f"Expected power({a}, {b}) to be {expected}, but got {result}"


def test_power_invalid() -> None:
    """
    Test that invalid power operations raise a ValueError (e.g., negative base with fractional exponent).
    """
    with pytest.raises(ValueError):
        power(-1, 0.5)


def test_power_nan_result() -> None:
    """
    Ensure that powering with NaN base produces a ValueError for non-real result.
    """
    with pytest.raises(ValueError):
        power(float('nan'), 1)
